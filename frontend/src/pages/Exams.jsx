...
const Exams = () => {
  ...
  const [expandedTeacherStats, setExpandedTeacherStats] = useState({});

  const toggleTeacherStats = (examId) => {
                                ...
                                {lessonExams.filter(ex => isTeacher || !ex.isHidden).map((ex, exIdx) => {
                                  ...
                                  <div key={ex.id} className="sub-exam-container" style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem', marginBottom: '0.5rem' }}>
                                    ...
                                    {teacherExamStats[ex.id]?.length > 0 && isTeacher && (
                                      <div className="sub-exam-teacher-stats" style={{ borderTop: '1px dashed #e5e7eb', marginTop: '0.4rem', paddingTop: '0.4rem', paddingLeft: '0.5rem' }}>
                                        <div 
                                          style={{ fontSize: '0.75rem', color: '#6b7280', margin: '0 0 0.3rem 0', fontWeight: '500', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                                          onClick={() => toggleTeacherStats(ex.id)}
                                        >
                                          {expandedTeacherStats[ex.id] ? <ChevronDown size={14} /> : <ChevronRight size={14} />} 
                                          Danh sách các học sinh đã làm ({teacherExamStats[ex.id].length})
                                        </div>
              ...
              <div key={exam.id} className={`exam-card card ${exam.isHidden ? 'opacity-60' : ''}`}>
                ...
                {teacherExamStats[exam.id]?.length > 0 && isTeacher && (
                  <div className="exam-history-section" style={{ borderTop: '1px dashed var(--border-color)', paddingTop: '0.6rem', marginTop: '0.6rem' }}>
                    <div 
                      style={{ fontSize: '0.75rem', color: '#6b7280', margin: '0 0 0.3rem 0', fontWeight: '500', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                      onClick={() => toggleTeacherStats(exam.id)}
                    >
                      {expandedTeacherStats[exam.id] ? <ChevronDown size={14} /> : <ChevronRight size={14} />} 
                      Danh sách các học sinh đã làm ({teacherExamStats[exam.id].length})
                    </div>
            ...
            <form onSubmit={handleSaveExam} className="modal-form-dual">
              <div className="editor-two-columns-layout">
                ...
                <div className="editor-left-column">
                  ...
                  <div className="points-config-compact" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center', marginBottom: '8px', background: 'var(--surface-color)', padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', flex: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <label style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: 0, whiteSpace: 'nowrap', fontWeight: '500' }}>TN 4 PA ({editorQuestions.filter(q => q.questionType === 'multiple_choice' || !q.questionType).length}):</label>
                        <input 
                          type="number" step="0.01" min="0" 
                          style={{ width: '45px', padding: '2px 4px', fontSize: '12px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', fontWeight: '600' }} 
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <label style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: 0, whiteSpace: 'nowrap', fontWeight: '500' }}>TL ngắn ({editorQuestions.filter(q => q.questionType === 'short_answer').length}):</label>
                        <input 
                          type="number" step="0.01" min="0" 
                          style={{ width: '45px', padding: '2px 4px', fontSize: '12px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', fontWeight: '600' }} 
                      ...
                      {editorQuestions.filter(q => q.questionType === 'true_false').length > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', borderLeft: '1px solid var(--border-color)', paddingLeft: '8px' }}>
                          <label style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: 0, whiteSpace: 'nowrap', fontWeight: '500' }}>Đúng/Sai ({editorQuestions.filter(q => q.questionType === 'true_false').length}):</label>
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <input type="number" step="0.01" min="0" title="Đúng 1 ý" style={{ width: '36px', padding: '2px', fontSize: '11px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', textAlign: 'center', fontWeight: '600' }} value={examFormData.pointsConfig?.trueFalse?.correct1 ?? 0.1} onChange={(e) => setExamFormData(prev => ({ ...prev, pointsConfig: { ...prev.pointsConfig, trueFalse: { ...prev.pointsConfig.trueFalse, correct1: parseFloat(e.target.value) || 0 } } }))} />
                            <input type="number" step="0.01" min="0" title="Đúng 2 ý" style={{ width: '36px', padding: '2px', fontSize: '11px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', textAlign: 'center', fontWeight: '600' }} value={examFormData.pointsConfig?.trueFalse?.correct2 ?? 0.25} onChange={(e) => setExamFormData(prev => ({ ...prev, pointsConfig: { ...prev.pointsConfig, trueFalse: { ...prev.pointsConfig.trueFalse, correct2: parseFloat(e.target.value) || 0 } } }))} />
                            <input type="number" step="0.01" min="0" title="Đúng 3 ý" style={{ width: '36px', padding: '2px', fontSize: '11px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', textAlign: 'center', fontWeight: '600' }} value={examFormData.pointsConfig?.trueFalse?.correct3 ?? 0.5} onChange={(e) => setExamFormData(prev => ({ ...prev, pointsConfig: { ...prev.pointsConfig, trueFalse: { ...prev.pointsConfig.trueFalse, correct3: parseFloat(e.target.value) || 0 } } }))} />
                            <input type="number" step="0.01" min="0" title="Đúng 4 ý" style={{ width: '36px', padding: '2px', fontSize: '11px', borderRadius: '4px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-color)', height: '24px', textAlign: 'center', fontWeight: '600' }} value={examFormData.pointsConfig?.trueFalse?.correct4 ?? 1.0} onChange={(e) => setExamFormData(prev => ({ ...prev, pointsConfig: { ...prev.pointsConfig, trueFalse: { ...prev.pointsConfig.trueFalse, correct4: parseFloat(e.target.value) || 0 } } }))} />
                          </div>
                        </div>
                      )}
                    </div>
