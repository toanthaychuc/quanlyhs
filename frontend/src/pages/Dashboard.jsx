            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {isTeacher ? (
                ...
                <>
                  ...
                  <span style={{ background: 'rgba(0,0,0,0.4)', padding: '0.2rem 0.6rem', borderRadius: '12px', fontSize: '0.7rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px', border: '1px solid rgba(255,255,255,0.1)' }}>Mã HS: {currentStudentId.replace(new RegExp(`^${studentInfo.classId}_`), '')}</span>
                </>
              )}
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingRight: '0.25rem', maxHeight: '280px', overflowY: 'auto' }}>
                {teacherStats.needsAttention.length === 0 ? (
                  ...
                  teacherStats.needsAttention.map((student, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', background: student.type === 'danger' ? 'rgba(239, 68, 68, 0.08)' : 'rgba(245, 158, 11, 0.08)', border: `1px solid ${student.type === 'danger' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(245, 158, 11, 0.2)'}`, borderRadius: 'var(--radius-md)' }}>
                      <div>
                        <div style={{ fontSize: '0.85rem', fontWeight: 700, color: student.type === 'danger' ? 'var(--danger-color)' : '#f59e0b' }}>
                          {student.studentName} <span style={{ fontSize: '0.75rem', fontWeight: 400 }}>({student.className})</span>
                        </div>
                        <div style={{ fontSize: '0.75rem', color: student.type === 'danger' ? 'var(--danger-color)' : '#f59e0b', marginTop: '0.1rem' }}>
                          {student.reason}
                        </div>
                      </div>
                      <button className={`btn ${student.type === 'danger' ? 'btn-danger' : 'btn-secondary'}`} style={{ padding: '0.25rem 0.6rem', fontSize: '0.75rem', backgroundColor: student.type === 'danger' ? 'var(--danger-color)' : '#f59e0b', color: 'white', border: 'none' }} onClick={() => navigate('/classes')}>
                        Xem lớp
                      </button>
                    </div>
                  ))
